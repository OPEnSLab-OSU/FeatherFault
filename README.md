...
While most projects should only need traces on the serial monitor, some (such as remote deployments) will need to log the data to other mediums. To do this, FeatherFault has the `FeatherFault::DidFault` and `FeatherFault::GetFault` functions, the documentation for which can be found TODO.
